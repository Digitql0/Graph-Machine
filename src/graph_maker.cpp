...
void render_graph(Graph g, float screenWidth, float screenHeight) {
  for (int i = 1; i < g.positive_values.size(); i++) {
    if (std::isnan(g.positive_values[i]) || std::isinf(g.positive_values[i])) {
      continue;
    }
  ...
  if (showXY) {
    MGL_drawLine(g.origin.x, g.origin.y, g.origin.x + 200, g.origin.y, 255, 0,
                 0);
    MGL_drawLine(g.origin.x, g.origin.y, g.origin.x, g.origin.y + 200, 0, 255,
                 0);
  }
}
